  ...
  get: (req, res) => {
    ...
    if (token) {
      ...
      Users.findOne({ where: { memberId: memberId } }).then((data) => {
        res.status(200);
        res.json({
          id: data.dataValues.memberId,
          name: data.dataValues.name,
          email: data.dataValues.email,
          phone: data.dataValues.phone,
          address: data.dataValues.address,
          money: data.dataValues.money,
          bankName: data.dataValues.bankName,
          bankNum: data.dataValues.bankNum,
        });
        res.end();
      });
    } else {
      //권한 없는 경우
      res.status(400).send("need user session");
      res.end();
    }
  },
    ...
    if (token) {
      ...
      Users.findOne({ where: { memberId: memberId } }).then((data) => {
        if (data) {
          ...
          console.log(total);
          Users.update(
            { money: data.dataValues.money + Number(amount) },
